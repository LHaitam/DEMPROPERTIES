          ...
          <motion.form key="f-prop" ref={formRef} onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="space-y-4">
              <div className="group">
                ...
                <input name="user_name" type="text" required className="w-full bg-transparent border-b border-stone-200 py-2 text-sm outline-none focus:border-[#a5694f] transition-all font-light" />
              </div>
              ...
              <div className="group">
                ...
                <input name="user_email" type="email" required className="w-full bg-transparent border-b border-stone-200 py-2 text-sm outline-none focus:border-[#a5694f] transition-all font-light" />
              </div>
              ...
              <div className="group">
                ...
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} required className="w-full bg-transparent border-b border-stone-200 py-2 text-sm outline-none focus:border-[#a5694f] transition-all resize-none font-light" />
              </div>
            </div>

            <button type="submit" disabled={status === "loading"} className="w-full bg-stone-900 text-white font-oswald uppercase tracking-[0.4em] text-[9px] py-4 mt-2 hover:bg-[#a5694f] transition-all flex justify-center items-center">
            ...
            <div className="pt-6 mt-4 border-t border-stone-50 text-center">
               <div className="flex flex-col gap-1">
                 ...
                 <span className="text-[8px] text-[#a5694f] uppercase tracking-widest italic font-medium">{propertyPrice}</span>
               </div>
            </div>
          </motion.form>
        ) : (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8 space-y-4">
            <div className="w-12 h-12 rounded-full border border-[#a5694f] flex items-center justify-center text-[#a5694f] mx-auto">
              <Check size={24} />
            </div>
            <h3 className="font-playfair italic text-xl">{t('contact.success.title')}</h3>
            <button onClick={() => setStatus("idle")} className="flex items-center gap-2 font-oswald text-[8px] uppercase tracking-[0.3em] text-[#a5694f] mx-auto">
